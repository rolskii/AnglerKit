...
export default function Moon() {
  ...
  const toggleAlarm = (timeStr) => {
    ...
    if (currentDayAlarms.time === timeStr) {
      ...
      // Also cancel the server-side alarm so a stale push notification doesn't
      // fire later for an alarm the user just turned off.
    ...
    // Register with the backend so a push notification still fires via
    // checkAlarms even if this tab (and the timer below) never gets the chance to.
    syncAlarmToServer(selectedDate, timeStr, offset, location);

    // Check alarm every minute — gives an immediate, no-permission-required
    // fallback while this tab stays open, on top of the server-side push above.
                          ...
                          {[0, 5, 10, 15].map((min) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                // Re-sync with the server under the new offset so the
                                // push notification fires at the newly selected lead time.
                                if (currentDayAlarms.time) {
                                  removeAlarmFromServer(selectedDate, currentDayAlarms.time).then(() =>
                                    syncAlarmToServer(selectedDate, currentDayAlarms.time, min, location)
                                  );
                                }
                              }}
