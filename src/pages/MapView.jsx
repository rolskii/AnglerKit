...
export default function MapView() {
  ...
  // Shared renderer for normalized access-point markers (Ontario + other
  // provinces). Renders a small violet dot; tapping opens AccessPointDialog.
  const renderAccessMarkers = (features, keyPrefix) => {
    ...
    return features.map((feat, idx) => {
      ...
      return (
        <div
          key={`${keyPrefix}-${idx}`}
          onClick={() => setSelectedAccessPoint(feat.properties)}
  ...
  useEffect(() => {
    ...
    if (showSeaMap) {
      if (!seaMapOverlayRef.current) {
        ...
        // OpenSeaMap's chart layer (seamarks: buoys, lights, harbour marks) —
        // the old {z}/{x}/{y} root path now serves blank placeholder tiles.
        // Seamark tiles are served up to zoom 18, so cap maximumZ to match.
        seaMapOverlayRef.current = new mapkit.TileOverlay('https://tiles.openseamap.org/seamark/{z}/{x}/{y}.png', { maximumZ: 18 });
    ...
    <div className="fixed inset-0 z-[4000] bg-background" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      >
        <Link to="/" className="flex items-center gap-2 pl-1 pr-2 rounded-lg hover:bg-accent/10 transition-colors shrink-0">
          ...
          <span className="font-heading font-semibold tracking-tight text-sm hidden sm:inline">HeronWise</span>
        </Link>
