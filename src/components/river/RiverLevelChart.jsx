      ...
      <div className="relative" style={{ height: CHART_HEIGHT }}>
        <svg viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} className="w-full cursor-pointer" style={{ height: CHART_HEIGHT, touchAction: 'none' }} preserveAspectRatio="none" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
          ...
          {overlayPath && overlayColor && (
            <path d={overlayPath} fill="none" stroke={overlayColor} strokeWidth="2" strokeLinecap="round" opacity="0.9" vectorEffect="non-scaling-stroke" />
          )}
          {lastReal && (
      ...
      <div className="flex items-center gap-3 mt-1 flex-wrap">
        ...
        {OVERLAY_COLORS[overlayRange] && overlayBuckets?.some(b => b.level != null || b.discharge != null) && overlayLabel && (
          <span className="inline-flex items-center gap-1 text-[11px] font-medium whitespace-nowrap" style={{ color: OVERLAY_COLORS[overlayRange] }}>
            <span className="inline-block w-3 border-t-2" style={{ borderColor: OVERLAY_COLORS[overlayRange] }} />
            {overlayLabel}
          </span>
        )}
      </div>
