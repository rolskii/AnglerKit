...
export default function SupplyCard({ item, onEdit, onDelete }) {
  ...
  return (
    <Card ref={cardRef} className="p-4 flex flex-col gap-3">
      <ImageGallery images={getItemImages(item)} featuredLabel={[item.brand, item.model, item.colour].filter(Boolean).join(" ") || item.name} featuredSubtitle="Supplies" featuredLink="/supplies" />
