...
export default function RodSelect({ rods, value, onChange }) {
  ...
  const selected = rods.find((r) => r.id === value) || null;
  const selectedLabel = selected ? selected.name : "";
  const placeholder = rods.length ? "Select a rod (optional)" : "No rods added yet";
  ...
  const tableContent = (
    <>
      ...
      <div className="max-h-64 overflow-y-auto">
        ...
        {rods.length === 0 && (
          <div className="px-3 py-4 text-sm text-muted-foreground text-center">No rods available</div>
        )}
        {rods.map((r) => (
          <button
            type="button"
    ...
    return (
      <Drawer open={open} onOpenChange={setOpen}>
        ...
        <DrawerContent className="max-h-[80vh]">
          <DrawerHeader>
            <DrawerTitle>Select a rod</DrawerTitle>
            <DrawerDescription>Choose a rod for this catch</DrawerDescription>
          </DrawerHeader>
